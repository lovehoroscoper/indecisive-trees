...
def get_idf_dictionary(query_dictionary, keyword_dictionary, title_dictionary, description_dictionary):
    """input dictionary is dict_description['idval'] = Counter({'75': 1, '1545': 1, '31': 1})"""
    ...
    for token in token_set:
        ...
        result_dictionary[token] = math.log(float(4)/1+occurence) # occurence+1: to adjust zero values e.g.) log(1) = 0 
    return result_dictionary
dic = Counter({'75': 1, '1545': 1, '31': 1})
def tf(dictionary):
    diction = {}
...
def cal_doc_similarity(query_id, keyword_id, title_id, description_id):
    ...
    for key in idf.keys() and tf_q.keys():
        ...
        result_dictionary_d[key] = idf[key]*tf_d[key]
        return result_dictionary
